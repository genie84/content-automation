...
def build_image_prompt(title: str, body_html: str) -> str:
    ...
    # 이 이미지는 지니(워드프레스)뿐 아니라 서현이 아빠(네이버)에도 그대로 재사용되므로
    # (2026-09-10 비용 절감 결정), 원문의 "지니의 생각" 콜아웃 라벨이 이미지에 그대로
    # 박히면 다른 페르소나 채널에서 어색하다 — 이미지에만 중립적인 라벨로 바꿔 넣는다
    # (워드프레스 본문 자체의 "지니의 생각" 문구는 건드리지 않음, 2026-09-14).
    plain_body = plain_body.replace("지니의 생각", "제 생각은요")
    return f"""\
...
- 이미지 비율: 가로형 와이드 (16:9)
- 원문에 "제 생각은요" 같은 필자 의견 콜아웃이 있다면, 그 라벨 문구를 그대로("제
  생각은요") 넣어라(다른 표현으로 바꾸지 마라).
"""
